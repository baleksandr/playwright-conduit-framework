...
test('Create article with check Screenshot', async ({ pageManager, testArticle, page }) => {
    ...
    await articlePage.makeVisualCheck(articlePage.likeButton, 'click_likes.png', {mask: []})
    await articlePage.makeVisualCheck(articlePage.articleCard, 'first_article.png')
});